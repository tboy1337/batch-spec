...
- Stock utilities such as `curl`, `tar`, `telnet`, and `wmic` remain in `builtin_commands` for detection, but may be absent on older or Feature-on-Demand-stripped hosts (`builtin_overlap_deprecated_notes`)
...
- Captured help under [`audit/cmd-help/`](../audit/cmd-help/) is a language-relevant subset (internals and high-impact stock tools such as `find` / `findstr` / `robocopy` / `fc` / `comp` / `attrib` / `tree` / `reg` / `replace` / `dpath` / `keys` / `mode` / `label` / `whoami` / `tasklist` / `taskkill` / `icacls` / `certutil` / `clip`), not a full dump of every `builtin_commands` name. Alias pairs share one help file (`cd`/`chdir`, `md`/`mkdir`, `rd`/`rmdir`, `del`/`erase`, `ren`/`rename`)
