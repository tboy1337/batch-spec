...
- Captured help under [`audit/cmd-help/`](../audit/cmd-help/) is a language-relevant subset (internals and a few high-impact externals such as `findstr` / `robocopy`), not a full dump of every `builtin_commands` name. Alias pairs share one help file (`cd`/`chdir`, `md`/`mkdir`, `rd`/`rmdir`, `del`/`erase`, `ren`/`rename`)
