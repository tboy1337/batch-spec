...
- Captured help under [`audit/cmd-help/`](../audit/cmd-help/) is a language-relevant subset (internals and high-impact stock tools such as `find` / `findstr` / `robocopy` / `fc` / `comp` / `attrib` / `tree`), not a full dump of every `builtin_commands` name. Alias pairs share one help file (`cd`/`chdir`, `md`/`mkdir`, `rd`/`rmdir`, `del`/`erase`, `ren`/`rename`)
