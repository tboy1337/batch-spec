...
- **Batch parameters** - `%*` and `%~` require Command Extensions (literal `*` / `~...` when off); base `%0`-`%9` work without extensions; `%10` is `%1` plus literal `0`; `%0` spelling mirrors CALL/invocation text (Explorer-style `cmd /c ""fullpath""` commonly yields a quoted full-path `%0`; relative CLI typing keeps the as-typed spelling); SHIFT `/n` and bare SHIFT; `%*` is `%1 %2 ...` (never includes `%0`) and is unaffected by SHIFT; empty quoted `""` occupies a slot (`%1` is `""`, `%~1` empty); unquoted args split on space/tab/comma/semicolon/equals (`a=b` → two slots while `%*` keeps `a=b`)
