...
- **Percent-tilde (`%~`)** - letter modifiers, path search (`%~$ENV:n`), bare quote-strip (`%~1`), and the multi-digit batveat (`%~10` is `%~1` plus literal `0`). `%~*` is semantically invalid (CALL /?) even when the letter-regex does not match it.
...
- **String ops** - substring with negative offsets/lengths and omitted length; `*` prefix replace; empty-defined substring batveat
...
- **Plain SET assignment** - spaces around `=` become part of the name and/or value; prefix query (`SET P`); missing name/prefix sets ERRORLEVEL 1
- **Environment variable names** - `=` forbidden in names; live cmd accepts `.`, `-`, and spaced names (prefer underscore-alnum for portability)
- **ECHO blank lines** - `ECHO.` / `ECHO:` write a blank line
- **CMD processor switches** - `/V` delayed expansion; `/E` Command Extensions
...
- **Redirection** - `>`, `>>`, `<`, `n>`, `>&` / `<&` handle duplication, NUL suppress, group redirects, leading redirects
