...
- **SET /A** - requires Command Extensions; operators with documented precedence, grouping, comma separator, hex/octal (`no_binary_literal`: not `0b` binary; `08`/`09` invalid), undefined-as-zero, bare names, 32-bit wrap on overflow, quoting rules; unary `!` interacts with delayed expansion; divide-by-zero leaves non-zero ERRORLEVEL
...
- **SET /P** - optional prompt; EOF/NUL keeps prior value; a blank input line (Enter with no text) also keeps the prior value (does not assign empty); a spaces-only line assigns those spaces; `SET /P var=<file` reads the first line only; pipe-side SET /P updates only the child cmd environment
...
- **SETLOCAL options** - Enable/Disable Extensions and DelayedExpansion (precedence over CMD `/E`/`/V`); nesting limit 32 per CALL level ("Maximum setlocal recursion level reached."); argument ERRORLEVEL probe; ENDLOCAL restores prior state; `endlocal & set "out=%in%"` same-line survive trick
...
- **Command chaining** - `&`, `&&`, `||`, `|`, and parenthesized groups; pipe sides run in child cmd contexts (delayed expansion defaults off in the child); `A && (B) || (C)` runs C when B fails even after successful A; bare trailing `&` inside `( )` is a syntax error
...
- **Directory commands** - CD `/D` changes drive; PUSHD/POPD stack; UNC temp drives from Z: down (PUSHD /?)
