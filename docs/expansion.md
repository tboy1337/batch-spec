...
- **ERRORLEVEL / CMDEXTVERSION** - `IF ERRORLEVEL n` means `>= n`; dynamic `%ERRORLEVEL%` env-var shadowing; CHOICE sets ERRORLEVEL to the 1-based choice ordinal (255 on tool error); CMDEXTVERSION starts at 1 and never true when extensions are off
