...
- **SET /P** - optional prompt; EOF/NUL keeps prior value; a blank input line (Enter with no text) also keeps the prior value (does not assign empty); a spaces-only line assigns those spaces; pipe-side SET /P updates only the child cmd environment
