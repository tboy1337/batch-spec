...
- **Percent-tilde (`%~`)** - letter modifiers (order-independent, case-insensitive), path search (`%~$ENV:n`, empty on miss), bare quote-strip (`%~1`), and the multi-digit batveat (`%~10` is `%~1` plus literal `0`). `%~*` is semantically invalid (CALL /?) even when the letter-regex does not match it.
- **Percent expansion** - in scripts, undefined `%name%` / `!name!` expand to empty; on the interactive prompt undefined `%name%` often remains literal; incomplete unclosed `%` forms are not successful expansions.
...
- **FOR variables / forms** - `%%i` in batch files, `%i` on the interactive command line; letter charset; `/D` `/R` `/L` `/F` forms; FOR variables are global
...
- **String ops** - substring with negative offsets/lengths and omitted length; replace-all; empty replacement deletes; `*` prefix replace; case-insensitive `%var:old=new%` search; missing/empty substring batveat (`%NOSUCH:~-1%` / after `SET name=` yields literal `~-1`)
- **SET /A** - operators with documented precedence, grouping, comma separator, hex/octal (not `0b` binary), undefined-as-zero, bare names, 32-bit wrap on overflow, quoting rules; unary `!` interacts with delayed expansion; divide-by-zero leaves non-zero ERRORLEVEL
...
- **CMD processor switches** - `/V` delayed expansion; `/E` Command Extensions; defaults (extensions on, delayed off); `/C` `/K` `/S` quote-stripping rules
- **SETLOCAL options** - Enable/Disable Extensions and DelayedExpansion (precedence over CMD `/E`/`/V`); nesting limits; argument ERRORLEVEL probe; ENDLOCAL restores prior state; `endlocal & set "out=%in%"` same-line survive trick
...
- **IF forms / parentheses** - base and extension predicates; EXIST for files and directories; quoted compare sides are string compares; string order is not raw ASCII; open `(` on the same line as the predicate (spaces allowed); ELSE same-line attachment
...
- **Batch parameters** - `%*`, `%0` spelling mirrors CALL text, SHIFT `/n` and bare SHIFT, `%*` unaffected by SHIFT; unquoted args split on space/tab/comma/semicolon/equals
